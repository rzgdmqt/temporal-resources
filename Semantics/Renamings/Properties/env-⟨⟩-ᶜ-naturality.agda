...
env-⟨⟩-ᶜ-nat zero p ρ = 
  ...
  ≡⟨ sym (⟨⟩-η-nat _) ⟩
    ⟨ 0 ⟩ᶠ ⟦ ρ ⟧ʳ ∘ᵐ η
  ∎
env-⟨⟩-ᶜ-nat {Γ ∷ A} {.(Γ ∷ A)} (suc τ) p id-ren = 
  begin
       (   env-⟨⟩-ᶜ {Γ} (suc τ) p
        ∘ᵐ fstᵐ)
    ∘ᵐ idᵐ
  ...
  ≡⟨ ∘ᵐ-congˡ (sym ⟨⟩-idᵐ) ⟩
    ...
    ∘ᵐ fstᵐ
  ∎
env-⟨⟩-ᶜ-nat {Γ ∷ A} {Γ'} (suc τ) p (_∘ʳ_ {Γ' = Γ''} ρ ρ') = 
  begin
       (   env-⟨⟩-ᶜ {Γ} (suc τ) p
        ∘ᵐ fstᵐ)
    ∘ᵐ ⟦ ρ' ⟧ʳ
  ...
  ≡⟨ ∘ᵐ-assoc _ _ _ ⟩
    ...
    ∘ᵐ env-⟨⟩-ᶜ {Γ''} (suc τ) (≤-trans p (ren-≤-ctx-time ρ'))
    ∘ᵐ ⟦ ρ ⟧ʳ
  ≡⟨ ∘ᵐ-congʳ (env-⟨⟩-ᶜ-nat (suc τ) (≤-trans p (ren-≤-ctx-time ρ')) ρ) ⟩
       ⟨ suc τ ⟩ᶠ ⟦ ρ' -ʳ suc τ ⟧ʳ
  ...
  ≡⟨ ∘ᵐ-congˡ (sym (⟨⟩-∘ᵐ  _ _)) ⟩
    ...
    ∘ᵐ env-⟨⟩-ᶜ {Γ'} (suc τ) (≤-trans p (≤-trans (ren-≤-ctx-time ρ') (ren-≤-ctx-time ρ)))
  ∎
env-⟨⟩-ᶜ-nat {Γ ∷ A} {.(Γ ∷ A ∷ _)} (suc τ) p wk-ren = 
  begin
       (   env-⟨⟩-ᶜ {Γ} (suc τ) p
        ∘ᵐ fstᵐ)
    ∘ᵐ fstᵐ
  ...
  ≡⟨ ∘ᵐ-congˡ (sym ⟨⟩-idᵐ) ⟩
    ...
    ∘ᵐ fstᵐ
  ∎
env-⟨⟩-ᶜ-nat {Γ ∷ A} {.Γ} (suc τ) p (var-ren x) = 
  begin
       (   env-⟨⟩-ᶜ {Γ} (suc τ) p
        ∘ᵐ fstᵐ)
  ...
  ≡⟨ ∘ᵐ-congˡ (sym ⟨⟩-idᵐ) ⟩
    ...
    ∘ᵐ env-⟨⟩-ᶜ {Γ} (suc τ) (≤-trans p (≤-reflexive refl))
  ∎
env-⟨⟩-ᶜ-nat {Γ ∷ A} {.((Γ ∷ A) ⟨ 0 ⟩)} (suc τ) p ⟨⟩-η⁻¹-ren = 
  begin
       env-⟨⟩-ᶜ {Γ ∷ A} (suc τ) p
    ∘ᵐ η⁻¹
  ≡⟨⟩
  ...
  ≡⟨ ∘ᵐ-congˡ (sym ⟨⟩-idᵐ) ⟩
    ...
    ∘ᵐ env-⟨⟩-ᶜ {(Γ ∷ A) ⟨ 0 ⟩} (suc τ) (≤-trans p (≤-reflexive (sym (+-identityʳ (ctx-time Γ)))))
  ∎
env-⟨⟩-ᶜ-nat {Γ ∷ A} {.(_ ∷ A)} (suc τ) p (cong-∷-ren {Γ' = Γ'} ρ) =
  begin
       (   env-⟨⟩-ᶜ {Γ} (suc τ) p
        ∘ᵐ fstᵐ)
    ∘ᵐ ⟨ ⟦ ρ ⟧ʳ ∘ᵐ fstᵐ ,
    ...
    ≡⟨ ∘ᵐ-congˡ (sym ⟨⟩-idᵐ) ⟩
      ...
      ∘ᵐ ⟨ τ' ⟩ᶠ (env-⟨⟩-ᶜ (suc τ ∸ τ')
                   (≤-trans (∸-monoˡ-≤ τ' (≤-trans p (≤-reflexive refl)))
                    (≤-reflexive (m+n∸n≡m (ctx-time Γ) τ'))))
  ∎
env-⟨⟩-ᶜ-nat {Γ ⟨ τ' ⟩} (suc τ) p (_∘ʳ_ {Γ' = Γ'} {Γ'' = Γ''} ρ ρ') = 
  begin
       env-⟨⟩-ᶜ {Γ ⟨ τ' ⟩} (suc τ) p
    ∘ᵐ ⟦ ρ ∘ʳ ρ' ⟧ʳ
  ≡⟨⟩
    ...
    ∘ᵐ ⟨ 0 ⟩ᶠ (   ⟨⟩-≤ (m≤n+m∸n (suc τ) τ')
               ...
               ∘ᵐ ⟨ τ' ⟩ᶠ (env-⟨⟩-ᶜ (suc τ ∸ τ')
                            (≤-trans
                              ...
                              (≤-reflexive (m+n∸n≡m (ctx-time Γ) τ')))))
  ∎
env-⟨⟩-ᶜ-nat {Γ ⟨ .(τ' + τ'') ⟩} (suc τ) p (⟨⟩-μ-ren {τ = τ'} {τ' = τ''}) = {!!} --with suc τ ≤? τ' + τ'' | suc τ ≤? τ''
--... | yes q | yes r =
--  {!!}
{-
  begin
...
env-⟨⟩-ᶜ-nat {Γ ⟨ τ' ⟩} (suc τ) p (⟨⟩-≤-ren {τ' = τ''} q) with suc τ ≤? τ' | suc τ ≤? τ''
... | yes r | yes s = 
  begin
       (   μ⁻¹
        ∘ᵐ ⟨⟩-≤ (≤-reflexive (m+[n∸m]≡n r)))
