...
record Past : Set₁ where
  ...
  field
    ...
    ⟨⟩-cong : ∀ {A B τ} {f g : A →ᵐ B} → f ≡ᵐ g → ⟨ τ ⟩ᶠ f ≡ᵐ ⟨ τ ⟩ᶠ g
    ...
    ⟨⟩-≤-trans : ∀ {A τ τ' τ''} → (p : τ ≤ τ') → (q : τ' ≤ τ'')
               → ⟨⟩-≤ {A} p ∘ᵐ ⟨⟩-≤ {A} q ≡ᵐ ⟨⟩-≤ {A} (≤-trans p q)

    ⟨⟩-≤-cong : ∀ {A τ τ'} → (p q : τ ≤ τ') → ⟨⟩-≤ {A} p ≡ᵐ ⟨⟩-≤ {A} q

    -- η and η⁻¹ are natural

    ⟨⟩-η-nat : ∀ {A B} → (f : A →ᵐ B) → ⟨ 0 ⟩ᶠ f ∘ᵐ η ≡ᵐ η ∘ᵐ f
    ⟨⟩-η⁻¹-nat : ∀ {A B} → (f : A →ᵐ B) → f ∘ᵐ η⁻¹ ≡ᵐ η⁻¹ ∘ᵐ ⟨ 0 ⟩ᶠ f

    -- μ and μ⁻¹ are natural

    ⟨⟩-μ-nat : ∀ {A B τ₁ τ₂} → (f : A →ᵐ B)
             → ⟨ τ₁ + τ₂ ⟩ᶠ f ∘ᵐ μ {A} ≡ᵐ μ {B} ∘ᵐ ⟨ τ₁ ⟩ᶠ (⟨ τ₂ ⟩ᶠ f)
    ⟨⟩-μ⁻¹-nat : ∀ {A B τ₁ τ₂} → (f : A →ᵐ B)
               → ⟨ τ₁ ⟩ᶠ (⟨ τ₂ ⟩ᶠ f) ∘ᵐ μ⁻¹ {A} ≡ᵐ μ⁻¹ {B} ∘ᵐ ⟨ τ₁ + τ₂ ⟩ᶠ f

    ⟨⟩-μ-≤ : ∀ {A τ₁ τ₂ τ₁' τ₂'} → (p : τ₁ ≤ τ₁') → (q : τ₂ ≤ τ₂')
           → ⟨⟩-≤ {A} (+-mono-≤ p q) ∘ᵐ μ {A} ≡ᵐ μ {A} ∘ᵐ ⟨ τ₁ ⟩ᶠ (⟨⟩-≤ {A} q) ∘ᵐ ⟨⟩-≤ {⟨ τ₂' ⟩ᵒ A} p

    -- TODO: derive the ones below from ⟨⟩-μ-≤

    ⟨⟩-μ-≤₁ : ∀ {A τ₁ τ₂ τ₁'} → (p : τ₁ ≤ τ₁')
            → ⟨⟩-≤ {A} (+-monoˡ-≤ τ₂ p) ∘ᵐ μ {A} ≡ᵐ μ {A} ∘ᵐ ⟨⟩-≤ {⟨ τ₂ ⟩ᵒ A} p
    ⟨⟩-μ-≤₂ : ∀ {A τ₁ τ₂ τ₂'} → (q : τ₂ ≤ τ₂')
            → ⟨⟩-≤ {A} (+-monoʳ-≤ τ₁ q) ∘ᵐ μ {A} ≡ᵐ μ {A} ∘ᵐ ⟨ τ₁ ⟩ᶠ (⟨⟩-≤ {A} q)

    ⟨⟩-μ⁻¹-≤₂ : ∀ {A τ₁ τ₂ τ₂'} → (q : τ₂ ≤ τ₂')
              → μ⁻¹ {A} ∘ᵐ ⟨⟩-≤ {A} (+-monoʳ-≤ τ₁ q) ≡ᵐ ⟨ τ₁ ⟩ᶠ (⟨⟩-≤ {A} q) ∘ᵐ μ⁻¹ {A}

    -- η is invertible

    ⟨⟩-η∘η⁻¹≡id : ∀ {A} → η {A} ∘ᵐ η⁻¹ ≡ᵐ idᵐ
    ⟨⟩-η⁻¹∘η≡id : ∀ {A} → η⁻¹ {A} ∘ᵐ η ≡ᵐ idᵐ

    -- μ is invertible

    ⟨⟩-μ∘μ⁻¹≡id : ∀ {A τ₁ τ₂} → μ {A} {τ₁} {τ₂} ∘ᵐ μ⁻¹ {A} {τ₁} {τ₂} ≡ᵐ idᵐ
    ⟨⟩-μ⁻¹∘μ≡id : ∀ {A τ₁ τ₂} → μ⁻¹ {A} {τ₁} {τ₂} ∘ᵐ μ {A} {τ₁} {τ₂} ≡ᵐ idᵐ

    -- Graded monad laws

    ⟨⟩-μ∘η≡id : ∀ {A τ} → μ {A} {0} {τ} ∘ᵐ η {⟨ τ ⟩ᵒ A} ≡ᵐ idᵐ
    ⟨⟩-μ∘Tη≡id : ∀ {A τ} → μ {A} {τ} {0} ∘ᵐ ⟨ τ ⟩ᶠ (η {A}) ≡ᵐ ⟨⟩-≤ {A} (≤-reflexive (+-identityʳ τ))
    ⟨⟩-μ∘μ≡≤∘μ∘Tμ : ∀ {A τ₁ τ₂ τ₃}
                  → μ {A} {τ₁ + τ₂} {τ₃} ∘ᵐ μ {⟨ τ₃ ⟩ᵒ A} {τ₁} {τ₂}
                 ≡ᵐ ⟨⟩-≤ {A} (≤-reflexive (+-assoc τ₁ τ₂ τ₃)) ∘ᵐ μ {A} {τ₁} {τ₂ + τ₃} ∘ᵐ ⟨ τ₁ ⟩ᶠ (μ {A} {τ₂} {τ₃})

    -- Graded monad laws (for inverses) (TODO: derive from above)

    ⟨⟩-Tη⁻¹∘ᵐμ⁻¹≡id : ∀ {A τ} →  ⟨ τ ⟩ᶠ (η⁻¹ {A}) ∘ᵐ μ⁻¹ {A} {τ} {0} ≡ᵐ ⟨⟩-≤ {A} (≤-reflexive (sym (+-identityʳ _)))
