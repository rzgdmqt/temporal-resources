...
record Past : Set₁ where
  ...
  field
    ...
    ⟨⟩-μ-nat : ∀ {A B τ₁ τ₂} → (f : A →ᵐ B)
             → ⟨ τ₁ + τ₂ ⟩ᶠ f ∘ᵐ μ {A} ≡ μ {B} ∘ᵐ ⟨ τ₁ ⟩ᶠ (⟨ τ₂ ⟩ᶠ f)
    ⟨⟩-μ⁻¹-nat : ∀ {A B τ₁ τ₂} → (f : A →ᵐ B)
               → ⟨ τ₁ ⟩ᶠ (⟨ τ₂ ⟩ᶠ f) ∘ᵐ μ⁻¹ {A} ≡ μ⁻¹ {B} ∘ᵐ ⟨ τ₁ + τ₂ ⟩ᶠ f

    ⟨⟩-μ-≤ : ∀ {A τ₁ τ₂ τ₁' τ₂'} → (p : τ₁ ≤ τ₁') → (q : τ₂ ≤ τ₂')
           → ⟨⟩-≤ {A} (+-mono-≤ p q) ∘ᵐ μ {A} ≡ μ {A} ∘ᵐ ⟨ τ₁ ⟩ᶠ (⟨⟩-≤ {A} q) ∘ᵐ ⟨⟩-≤ {⟨ τ₂' ⟩ᵒ A} p

    -- TODO: derive the ones below from ⟨⟩-μ-≤

    ⟨⟩-μ-≤₁ : ∀ {A τ₁ τ₂ τ₁'} → (p : τ₁ ≤ τ₁')
            → ⟨⟩-≤ {A} (+-monoˡ-≤ τ₂ p) ∘ᵐ μ {A} ≡ μ {A} ∘ᵐ ⟨⟩-≤ {⟨ τ₂ ⟩ᵒ A} p
    ⟨⟩-μ-≤₂ : ∀ {A τ₁ τ₂ τ₂'} → (q : τ₂ ≤ τ₂')
            → ⟨⟩-≤ {A} (+-monoʳ-≤ τ₁ q) ∘ᵐ μ {A} ≡ μ {A} ∘ᵐ ⟨ τ₁ ⟩ᶠ (⟨⟩-≤ {A} q)

    ⟨⟩-μ⁻¹-≤₂ : ∀ {A τ₁ τ₂ τ₂'} → (q : τ₂ ≤ τ₂')
              → μ⁻¹ {A} ∘ᵐ ⟨⟩-≤ {A} (+-monoʳ-≤ τ₁ q) ≡ ⟨ τ₁ ⟩ᶠ (⟨⟩-≤ {A} q) ∘ᵐ μ⁻¹ {A}
    ...
    ⟨⟩-μ∘μ⁻¹≡id : ∀ {A τ₁ τ₂} → μ {A} {τ₁} {τ₂} ∘ᵐ μ⁻¹ {A} {τ₁} {τ₂} ≡ idᵐ
    ⟨⟩-μ⁻¹∘μ≡id : ∀ {A τ₁ τ₂} → μ⁻¹ {A} {τ₁} {τ₂} ∘ᵐ μ {A} {τ₁} {τ₂} ≡ idᵐ

    -- Graded monad laws

    ⟨⟩-μ∘η≡id : ∀ {A τ} → μ {A} {0} {τ} ∘ᵐ η {⟨ τ ⟩ᵒ A} ≡ idᵐ
    ⟨⟩-μ∘Tη≡id : ∀ {A τ} → μ {A} {τ} {0} ∘ᵐ ⟨ τ ⟩ᶠ (η {A}) ≡ ⟨⟩-≤ {A} (≤-reflexive (+-identityʳ τ))
    ⟨⟩-μ∘μ≡≤∘μ∘Tμ : ∀ {A τ₁ τ₂ τ₃}
                  → μ {A} {τ₁ + τ₂} {τ₃} ∘ᵐ μ {⟨ τ₃ ⟩ᵒ A} {τ₁} {τ₂}
                 ≡ ⟨⟩-≤ {A} (≤-reflexive (+-assoc τ₁ τ₂ τ₃)) ∘ᵐ μ {A} {τ₁} {τ₂ + τ₃} ∘ᵐ ⟨ τ₁ ⟩ᶠ (μ {A} {τ₂} {τ₃})
