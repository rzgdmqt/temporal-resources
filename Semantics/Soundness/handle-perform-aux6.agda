...
handle-perform-sound-aux6 {Γ} {A} {B} {τ} {τ'} op V M H N =
  ...
  ≡⟨ ∘ᵐ-congʳ (∘ᵐ-congˡ (cong₂ ⟨_,_⟩ᵐ refl (cong₂ ⟨_,_⟩ᵐ refl (∘ᵐ-congˡ
    ...
    ∘ᵐ ⟨ idᵐ , ⟨ ⟦ V ⟧ᵛᵗ , ⟨ idᵐ , η⊣ ⟩ᵐ ⟩ᵐ ⟩ᵐ
  ≡⟨ ∘ᵐ-congʳ (∘ᵐ-congˡ (cong₂ ⟨_,_⟩ᵐ refl (cong₂ ⟨_,_⟩ᵐ refl (∘ᵐ-congˡ
      (cong [ op-time op ]ᶠ (cong curryᵐ (trans (sym (∘ᵐ-assoc _ _ _)) (∘ᵐ-congˡ
        (trans (sym (uncurryᵐ-nat _ _)) (cong uncurryᵐ (trans (∘ᵐ-assoc _ _ _)
    ...
    ∘ᵐ ⟨ idᵐ , ⟨ ⟦ V ⟧ᵛᵗ , ⟨ idᵐ , η⊣ ⟩ᵐ ⟩ᵐ ⟩ᵐ
  ≡⟨ ∘ᵐ-congʳ (∘ᵐ-congˡ (cong₂ ⟨_,_⟩ᵐ refl (cong₂ ⟨_,_⟩ᵐ refl (∘ᵐ-congˡ
      (cong [ op-time op ]ᶠ (cong curryᵐ (∘ᵐ-congˡ (cong uncurryᵐ (∘ᵐ-congʳ
        (trans (∘ᵐ-congʳ (trans (sym (⟨⟩ᵢᵐ-∘ᵐ _ _)) (cong ⟨_⟩ᵢᵐ (fun-ext (λ op → sym (⟨⟩ᵢᵐ-∘ᵐ _ _))))))
          (trans (sym (mapⁱˣᵐ-⟨⟩ᵐ _ _)) (sym (trans (sym (mapⁱˣᵐ-⟨⟩ᵐ _ _))
            (cong ⟨_⟩ᵢᵐ (fun-ext (λ op →
              trans (sym (mapⁱˣᵐ-⟨⟩ᵐ _ _)) (sym (trans (sym (mapⁱˣᵐ-⟨⟩ᵐ _ _))
                (cong ⟨_⟩ᵢᵐ (fun-ext (λ τ''' →
                  trans (∘ᵐ-congʳ (∘ᵐ-identityˡ _)) (sym (trans (∘ᵐ-identityʳ _)
                    (sym (trans (∘ᵐ-assoc _ _ _) (∘ᵐ-congʳ (trans (sym (curryᵐ-nat _ _))
                      (cong curryᵐ (trans (∘ᵐ-assoc _ _ _) (∘ᵐ-congʳ
                        (trans (∘ᵐ-assoc _ _ _) (trans (∘ᵐ-congʳ (sym (⟨⟩ᵐ-∘ᵐ _ _ _)))
                          (trans (sym (⟨⟩ᵐ-∘ᵐ _ _ _)) (sym (trans (sym (⟨⟩ᵐ-∘ᵐ _ _ _))
                            (cong₂ ⟨_,_⟩ᵐ
                              (trans (∘ᵐ-assoc _ _ _) (trans (∘ᵐ-congʳ (⟨⟩ᵐ-fstᵐ _ _))
                                (trans (sym (mapˣᵐ-⟨⟩ᵐ _ _ _ _)) (sym (trans (∘ᵐ-assoc _ _ _)
                                  (trans (∘ᵐ-congʳ (⟨⟩ᵐ-fstᵐ _ _)) (trans (∘ᵐ-congʳ (sym (⟨⟩ᵐ-∘ᵐ _ _ _)))
                                    (trans (sym (mapˣᵐ-⟨⟩ᵐ _ _ _ _))
                                      (cong₂ ⟨_,_⟩ᵐ
                                        (trans (∘ᵐ-congʳ (⟨⟩ᵐ-fstᵐ _ _)) (sym (∘ᵐ-assoc _ _ _)))
                                        (∘ᵐ-congʳ (trans (∘ᵐ-assoc _ _ _) (∘ᵐ-congʳ (trans (⟨⟩ᵐ-sndᵐ _ _) (∘ᵐ-identityˡ _))))))))))))))
                              (trans (∘ᵐ-assoc _ _ _) (sym (trans (∘ᵐ-assoc _ _ _) (∘ᵐ-congʳ
                                (trans (⟨⟩ᵐ-sndᵐ _ _) (trans (∘ᵐ-assoc _ _ _) (trans (∘ᵐ-congʳ (⟨⟩ᵐ-sndᵐ _ _))
                                  (trans (∘ᵐ-congʳ (∘ᵐ-identityˡ _)) (sym (⟨⟩ᵐ-sndᵐ _ _)))))))))))))))))))))))))))))))))))))))))))))) ⟩
       ×ᵐ-assoc⁻¹
    ∘ᵐ ⟨ fstᵐ ,
         ⟨    fstᵐ
           ∘ᵐ sndᵐ ,
